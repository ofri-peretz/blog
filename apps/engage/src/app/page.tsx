...
export default function Page() {
  ...
  const openPerson = useCallback(async (u: string) => {
    ...
    try {
      const r = await fetch(`/api/person?u=${encodeURIComponent(u)}`, {
        cache: "no-store",
      });
  ...
  useEffect(() => {
    load();
    pull("insights", "/api/insights", (v: any) => setInsights(v)).catch(() =>
      setInsights({ metrics: {}, metricsError: "unreachable", authors: [] }),
    );
    pull("network", "/api/network", (v: any) => setGraph(v)).catch(() =>
      setGraph(null),
    );
    ...
    pull("sources", "/api/sources", (v: any) => setSources(v)).catch(() =>
      setSources({}),
    );
    pull("audience", "/api/audience", (v: any) => setAudience(v)).catch(() =>
      setAudience({ hours: [], zones: [], error: "unreachable" }),
    );
    pull("people", "/api/people", (v: any) => setPeople(v)).catch(() =>
      setPeople({ people: [] }),
    );
    pull("board", "/api/board", (v: any) => setBoard(v)).catch(() =>
      setBoard({ prs: [] }),
    );
    pull("eco", "/api/ecosystem", (v: any) => setEco(v)).catch(() =>
      setEco({}),
    );
    pull("web", "/api/articles", (v: any) => setWeb(v)).catch(() =>
      setWeb({ nodes: [] }),
    );
    pull("catalog", "/api/plugins", (v: any) => setCatalog(v)).catch(() =>
      setCatalog({ plugins: [] }),
    );
    pull("corr", "/api/correlate", (v: any) => setCorr(v)).catch(() =>
      setCorr({ results: [], blocked: "unreachable" }),
    );
    pull(`trends:day`, "/api/trends?grain=day", (v: any) => setTrends(v)).catch(
      () => setTrends(null),
    );
    pull("bench", "/api/benchmark", (v: any) => setBench(v)).catch(() =>
      setBench(null),
    );
    pull("standing", "/api/standing", (v: any) => setStanding(v)).catch(() =>
      setStanding({ today: null, history: [], error: "unreachable" }),
    );
    pull("profile", "/api/profile", (v: any) => setProfile(v)).catch(() =>
      setProfile({ hint: "unreachable" }),
    );
    pull("impact", "/api/impact", (v: any) => setImpact(v)).catch(() =>
      setImpact(null),
    );
    ...
    pull("founders", "/api/founders", (v: any) => setFounders(v)).catch(() =>
      setFounders(null),
    );
  ...
  const skipItem = useCallback(
    async (n: number) => {
      ...
      try {
        ...
        if (!j.ok) throw new Error(j.error ?? `HTTP ${r.status}`);
        setState((st) =>
          st ? { ...st, items: st.items.filter((_, k) => k !== n) } : st,
        );
        if (n < i) setI((k) => Math.max(0, k - 1));
        setActErr(null);
      } catch (e: any) {
        setActErr(
          `@${it.article.author} was not recorded (${String(e?.message ?? e).slice(0, 80)}).`,
        );
      }
    },
    [state, i],
    ...
    <main className="mx-auto flex max-w-5xl flex-col gap-8 px-5 py-10 pb-24">
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Stalled feeds {alerts?.firing ? `· ${alerts.firing}` : ""}
              {alerts?.high ? ` · ${alerts.high} high` : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("alerts", "/api/alerts", (v: any) => setAlerts(v), true)
              }
              at={at.alerts ?? null}
              busy={!!busy.alerts}
            />
      ...
      {bands?.results?.length ? (
        <p className="-mt-4 font-mono text-[11px] text-[var(--muted-foreground)]">
          control bands ·{" "}
          {bands.results
            .map((r: any) => `${r.id} ${r.tier ?? "ok"}`)
            .join(" · ")}{" "}
          · as of {String(bands.at).slice(0, 10)}
        </p>
      ) : null}
      ...
      {nba.length > 0 && (
        ...
        >
            ...
            {nba.map((r) => {
              ...
              return (
                <li
                  ...
                  className="flex items-center gap-2 rounded border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-[12.5px]"
                >
                  ...
                  <span
                    className="hidden shrink-0 text-[11px] text-[var(--muted-foreground)] md:inline"
                    title={r.why}
                  >
                    {r.why}
                  </span>
                  ...
                  <button
                    onClick={() => {
                      if (r.source === "thread") {
                        setRi(r.index);
                        setReply(threads?.[r.index]?.draft ?? "");
                        setFocus("replies");
                      } else {
                        setI(r.index);
                        setFocus("queue");
          ...
          <p className="mt-2 text-[12px] text-[var(--muted-foreground)]">
            Ranked by what each action does to standing: closing a mutual tie
            first, an untouched core node next, then any new author, with
            waiting time on top. Jump puts the card in place; Enter sends it.
          </p>
        </Collapse>
      )}

      <Collapse id="s1" head={<>Up next</>}>
        {!state ? (
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Impact score
              {impact?.score != null ? ` · ${impact.score} / 100` : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("impact", "/api/impact", (v: any) => setImpact(v), true)
              }
              at={at.impact ?? null}
              busy={!!busy.impact}
              source={impact?.day ?? null}
            />
          </>
        }
      >
          ...
          <div className="flex flex-col gap-4">
              ...
              {impact.pillars.map((p: any) => (
                ...
                >
                  ...
                  <div className="mt-2 h-1.5 w-full rounded bg-[var(--border)]">
                    <div
                      className="h-1.5 rounded bg-[var(--primary)]"
                      style={{
                        width: `${Math.round((100 * p.points) / p.max)}%`,
                      }}
                    />
                  </div>
              ...
              <table className="w-full text-[12px]">
                <thead>
                  <tr className="font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--muted-foreground)]">
                    ...
                    <th className="px-2 py-1.5 text-right">floor → target</th>
                    <th className="px-2 py-1.5 text-right">points</th>
                    <th className="px-2 py-1.5 text-left">source</th>
                  </tr>
                </thead>
                <tbody>
                  {impact.pillars.flatMap((p: any) =>
                    p.metrics.map((m: any) => (
                      <tr
                        key={m.id}
                      ...
                      >
                        ...
                        <td className="px-2 py-1 text-right font-mono text-[11px] text-[var(--muted-foreground)]">
                          {m.floor} → {m.target}
                        </td>
                        <td className="px-2 py-1 text-right tabular-nums">
                          {Math.round(m.unitScore * 100)}%
                        </td>
                        <td className="px-2 py-1 font-mono text-[10px] text-[var(--muted-foreground)]">
                          {m.source}
                        </td>
            ...
            {impact.climb ? (
              <div className="text-[12px]">
                ...
                rank <b>{impact.climb.rank ?? "—"}</b> of {impact.climb.authors}
                {impact.climb.next
                  ? ` · +${impact.climb.next.reactionsNeeded} reactions to top ${impact.climb.next.level} (${impact.climb.plan.articlesAtTop10Rate ?? "—"} articles at the top-10 rate)`
                  : ""}{" "}
                ·{" "}
                <Link href="/league" className="text-[var(--primary)]">
                  the climb →
                </Link>
              </div>
            ) : null}
            {impact.league?.length ? (
              <div className="text-[12px] text-[var(--muted-foreground)]">
                <span className="font-mono text-[10px] uppercase tracking-[0.08em]">
                  arena, 30-day top-300 by tag ·{" "}
                </span>
                {impact.league
                  .map(
                    (t: any) =>
                      `#${t.tag} ${t.rank ? `${t.rank}/${t.authors}` : "absent"}${
                        t.above?.length
                          ? ` (next up: ${t.above
                              .slice(-2)
                              .map((l: any) => `@${l.author} ${l.reactions}rx`)
                              .join(", ")})`
                          : ""
                      }`,
                  )
            ...
            {impact.downstream ? (
              <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3 text-[12px]">
                <div className="font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--muted-foreground)]">
                  downstream · does it reach the work?
                </div>
                <div className="mt-2 grid gap-3 md:grid-cols-3">
                  <div>
                    <div className="text-[11px] text-[var(--muted-foreground)]">
                      npm downloads, 7 days after over 7 before · scope beside
                      each
                    </div>
                    {impact.downstream.lifts.length ? (
                      <ul className="mt-1 flex flex-col gap-0.5">
                        {impact.downstream.lifts.map((l: any) => (
                          <li
                            key={l.slug}
                            className="flex items-baseline gap-2 truncate"
                          >
                            <span
                              className={`w-14 shrink-0 text-right font-mono tabular-nums ${l.lift >= 20 ? "text-[var(--success)]" : l.lift < 0 ? "text-[var(--warning)]" : ""}`}
                            >
                              {l.lift > 0 ? "+" : ""}
                              {l.lift}%
                            </span>
                            <span className="truncate" title={l.slug}>
                              {l.slug
                                .replace(/-[a-z0-9]{3,4}$/, "")
                                .replace(/-/g, " ")}
                            </span>
                            <span className="shrink-0 font-mono text-[10px] text-[var(--muted-foreground)]">
                              {l.scope} · {l.pre}→{l.post}/d
                            </span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="mt-1 text-[11px] text-[var(--muted-foreground)]">
                      {impact.downstream.liftsInWindow} article
                      {impact.downstream.liftsInWindow === 1 ? "" : "s"}{" "}
                      published in the last 30 days carry a lift; the metric is
                      their median. Ecosystem lift is a signal, not a cause.
                    </div>
                  </div>
                  <div>
                    <div className="text-[11px] text-[var(--muted-foreground)]">
                      people dev.to sent us, 30 days
                    </div>
                    {impact.downstream.reach ? (
                      <>
                        <div className="mt-1 text-xl font-semibold tabular-nums">
                          {impact.downstream.reach.sessions ?? "—"}
                          <span className="text-[12px] font-normal text-[var(--muted-foreground)]">
                            {" "}
                            human sessions
                          </span>
                        </div>
                        {impact.downstream.reach.clicks ? (
                          <div className="tabular-nums">
                            {impact.downstream.reach.clicks.humans} human ·{" "}
                            {impact.downstream.reach.clicks.bots} bot clicks on
                            /go/ links
                          </div>
                        ) : null}
                        <div className="mt-1 text-[11px] text-[var(--muted-foreground)]">
                          dev.to strips referrers; counted by the utm tag the
                          /go/ links carry. Clicks carry the visitor&apos;s user
                          agent since {impact.downstream.reach.classifiedSince};
                          before that every click read as a bot, so the human
                          count runs low until{" "}
                          {new Date(
                            Date.parse(
                              impact.downstream.reach.classifiedSince,
                            ) +
                              30 * 86_400_000,
                          )
                            .toISOString()
                  ...
                  <div>
                    <div className="text-[11px] text-[var(--muted-foreground)]">
                      followers who ever commented
                    </div>
                    {impact.downstream.followers ? (
                      <>
                        <div className="mt-1 text-xl font-semibold tabular-nums">
                          {impact.downstream.followers.commented ?? "—"}
                          <span className="text-[12px] font-normal text-[var(--muted-foreground)]">
                            {" "}
                            of {impact.downstream.followers.total ?? "—"}{" "}
                            followers
                          </span>
                        </div>
                        <div className="tabular-nums">
                          {impact.downstream.followers.commenters ?? "—"}{" "}
                          distinct people have commented on us; the rest do not
                          follow.
                        </div>
                        <div className="mt-1 text-[11px] text-[var(--muted-foreground)]">
                          The honest proxy for followers who read. It moves when
                          a commenter follows or a follower comments, nothing
                          else.
                        </div>
                      </>
                    ) : null}
            ...
            <p className="text-[12px] text-[var(--muted-foreground)]">
              {impact.measured} of {impact.total} metrics measured. Unmeasured
              scores zero on purpose: it is a fact about our instrumentation,
              not about the author. Followers, lifetime totals and badges are
              excluded.
            </p>
          </div>
        )}
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              What the founders are building
              {founders?.brief?.building?.length
                ? ` · ${founders.brief.building.length}`
                : ""}
            </span>
            <Refresh
      ...
      >
        {!founders ? (
          ...
          <Callout tone="warn" title="No brief yet">
            {founders.hint ??
              founders.error ??
              "The loop writes it once a day."}
          </Callout>
        ) : (
          <div className="flex flex-col gap-4">
            <p className="text-[15px] leading-snug">
              {founders.brief.headline}
            </p>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {(founders.brief.building ?? []).map((b: any) => (
                <article
                  ...
                  className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3"
                >
                  <div className="flex flex-wrap items-baseline gap-2 font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--muted-foreground)]">
                    <span>@{b.author}</span>
                    <span>{b.date}</span>
                  </div>
                  <a
                    ...
                    target="_blank"
                    rel="noopener"
                    className="mt-1 block text-[14px] font-semibold leading-snug hover:text-[var(--primary)]"
                  >
                    {b.title}
                  </a>
                  ...
                  {b.our_move && b.our_move.toLowerCase() !== "none" && (
                    <p className="mt-1.5 rounded border border-[var(--primary)]/40 px-2 py-1 text-[12.5px]">
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--muted-foreground)]">
                      ...
                      <th className="px-2 py-1.5 text-left">dates</th>
                      <th className="px-2 py-1.5 text-left">what it rewards</th>
                      <th className="px-2 py-1.5 text-left">our move</th>
                    </tr>
                  </thead>
                  <tbody>
                    {founders.brief.programs.map((p: any) => (
                      <tr
                        key={p.name}
                      ...
                      >
                        ...
                        <td className="px-2 py-1">{p.what_it_rewards}</td>
                        <td className="px-2 py-1">
                          {p.our_move && p.our_move.toLowerCase() !== "none" ? (
                            p.our_move
                          ) : (
                            <span className="text-[var(--muted-foreground)]">
                              —
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {founders.brief.what_they_reward?.length ? (
                <div>
                  ...
                  <ul className="mt-1 list-disc pl-4 text-[13px]">
                    {founders.brief.what_they_reward.map((x: string) => (
                      <li key={x}>{x}</li>
                    ))}
                  </ul>
                </div>
              ...
              {founders.brief.watch?.length ? (
                <div>
                  ...
                  <ul className="mt-1 list-disc pl-4 text-[13px]">
                    {founders.brief.watch.map((x: string) => (
                      <li key={x}>{x}</li>
                    ))}
                  </ul>
                </div>
            ...
            <details className="text-[12px]">
              <summary className="cursor-pointer font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--muted-foreground)]">
                the {founders.posts?.length ?? 0} staff posts behind this
                {founders.days ? `, ${founders.days} days` : ""}
              </summary>
              <ul className="mt-2 flex flex-col gap-0.5">
                {(founders.posts ?? []).map((p: any) => (
                  <li key={p.id} className="flex items-baseline gap-2">
                    <span className="w-20 shrink-0 font-mono text-[10px] text-[var(--muted-foreground)]">
                      {p.kind}
                    </span>
                    <span className="w-32 shrink-0 font-mono text-[11px]">
                      @{p.author}
                    </span>
                    <span className="w-20 shrink-0 font-mono text-[10px] text-[var(--muted-foreground)]">
            ...
            <p className="text-[12px] text-[var(--muted-foreground)]">
              Written by the morning batch from the staff's own public posts;
              the model never runs on this page. Sources are linked; nothing
              here is a program you have to enter.
            </p>
          </div>
        )}
      </Collapse>
          ...
          <>
            <span>
              ...
              {levers?.levers?.length
                ? ` · ${levers.levers.length} levers`
                : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("levers", "/api/levers", (v: any) => setLevers(v), true)
              }
              at={at.levers ?? null}
              busy={!!busy.levers}
              source={levers?.cachedAt ?? null}
            />
          </>
        }
      >
        {!levers ? (
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {(
              ["views14", "comments14", "reactions_per_100_views"] as const
            ).map((o) => (
              <div
                key={o}
                className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3"
              >
                <div className="font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--muted-foreground)]">
                  {o === "views14"
                    ? "views, first 14 days"
                    : o === "comments14"
                      ? "comments, first 14 days"
                      : "reactions per 100 views"}
                </div>
                <ul className="mt-2 flex flex-col gap-1 text-[12px]">
                  {levers.levers
                    .filter((l: any) => l.outcome === o)
                    .slice(0, 8)
                    .map((l: any) => (
                      <li
                        key={l.feature}
                      ...
                      >
                        <span
                          className={`w-12 shrink-0 text-right font-mono tabular-nums ${l.r > 0 ? "text-[var(--success)]" : "text-[var(--warning)]"}`}
                        >
                          {l.r > 0 ? "+" : ""}
                          {l.r}
                        </span>
                  ...
                  {levers.levers.filter((l: any) => l.outcome === o).length ===
                    0 && (
                    <li className="text-[var(--muted-foreground)]">
                      nothing above the threshold (n ≥ 20, |r| ≥ 0.2)
                    </li>
                  )}
                </ul>
              </div>
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Profile
              {profile?.readers?.viewsPerDay7 != null
                ? ` · ${profile.readers.viewsPerDay7} views/day`
                : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("profile", "/api/profile", (v: any) => setProfile(v), true)
              }
              at={at.profile ?? null}
              busy={!!busy.profile}
              source={profile?.asOf ?? null}
            />
          </>
        }
      >
          ...
          items={[
            {
              key: "views7",
              label: "Views / day",
              value: profile?.readers?.viewsPerDay7 ?? null,
              note: "7-day mean, dev.to analytics",
            },
            {
              key: "read7",
              label: "Read time",
              value: profile?.readers?.readTimeAvgS7 ?? null,
              unit: "s",
              note: "average per view, 7 days",
            },
            {
              key: "rx100",
              label: "Reactions / 100 views",
              value: profile?.resonance?.reactionsPer100Views30 ?? null,
              note: "30 days",
            },
            {
              key: "cm100",
              label: "Comments / 100 views",
              value: profile?.resonance?.commentsPer100Views30 ?? null,
              note: "30 days",
            },
            {
              key: "fol7",
              label: "Follows / day",
              value: profile?.followers?.followsPerDay7 ?? null,
              note: "7-day mean, all sources",
            },
            {
              key: "onb",
              label: "Onboarding share",
              value: profile?.followers?.onboardingShare30 ?? null,
              unit: "%",
              note: "follows from same-day accounts, 30 days",
            },
            {
              key: "fread",
              label: "Followers who commented",
              value: profile?.followers?.commentersWhoFollow ?? null,
              note: `of ${profile?.followers?.total ?? "—"} followers`,
            },
            {
              key: "onbt",
              label: "Onboarding followers",
              value: profile?.followers?.onboardingTotal ?? null,
              note: "accounts created the day they followed",
            },
          ]}
        />
        {profile?.referrers?.length ? (
          <p className="mt-3 font-mono text-[11px] text-[var(--muted-foreground)]">
            referrers ·{" "}
            {profile.referrers
              .map((r: any) => `${r.domain} ${r.views.toLocaleString()}`)
              .join(" · ")}
          </p>
        ) : null}
        <p className="mt-2 text-[12.5px] text-[var(--muted-foreground)]">
          Followers are not readers: dev.to suggests authors to every new
          account, and those follows arrive on days with fewer views than
          follows. Read time and comments per view are the numbers that move
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Standing
              {standing?.today?.rank_nonstaff
                ? ` · #${standing.today.rank_nonstaff} among non-staff`
                : ""}
            </span>
            <Refresh
              onClick={() =>
              ...
              at={at.standing ?? null}
              source={standing?.graphFetchedAt ?? null}
              busy={!!busy.standing}
            />
          </>
        }
        ...
        <StatStrip
          ...
          items={(
            ...
            return {
              ...
              label,
              note,
              value: standing?.today?.[k] ?? null,
              prior: typeof prior === "number" ? prior : null,
            };
          })}
        />
        {standing?.error && (
          <Callout tone="warn" title="No standing yet" className="mt-3">
            {standing.error}
          </Callout>
        )}
        <p className="mt-3 text-[12.5px] text-[var(--muted-foreground)]">
          Every number is a count of comment edges or threads that exist on
          dev.to. Followers and reactions are absent on purpose: the first is
          invisible for other accounts, the second carries no &ldquo;who&rdquo;.
          Rank compares within one crawl policy only.
        </p>
      </Collapse>
      ...
      <Collapse
        ...
        head={
          <>
            <span>DEV community network</span>
            <Refresh
              onClick={() =>
                pull("network", "/api/network", (v: any) => setGraph(v), true)
              }
              at={at.network ?? null}
              source={graph?.fetchedAt ?? null}
              busy={!!busy.network}
            />
          </>
        }
      ...
      {person && (
        <Collapse id="s4" head={<>Author focus</>}>
          {person.loading ? (
            <Skel rows={4} />
          ) : (
            <Person
      ...
      >
        ...
        */}
        ...
        <p className="text-[12.5px] text-[var(--muted-foreground)]">
          Ranked by who <b>talked back</b> first, then by volume — a reply is
          the only signal here we did not manufacture. Amber means they answered
          us and we have not answered them. A tall <b>Drafted</b> with 0%
          conversion is a relationship the queue keeps proposing and nobody ever
          completes.
        </p>
      </Collapse>
      {/* ── Reply inbox ──────────────────────────────────────────────────── */}
      ...
      <Collapse
        ...
        head={
          <>
            ...
            <Refresh
              onClick={() =>
                pull("sources", "/api/sources", (v: any) => setSources(v), true)
              }
              at={at.sources ?? null}
              source={sources?.asOf ?? null}
              busy={!!busy.sources}
            />
          </>
        }
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Plugin promotion{" "}
              {sources?.promotion?.prs?.length
                ? `· ${sources.promotion.prs.length} PRs`
                : ""}
            </span>
            <Refresh
              onClick={() =>
      ...
      >
        {sources ? (
          <Promotion prs={sources.promotion?.prs ?? []} />
        ) : (
          <Skel rows={4} />
        )}
      </Collapse>
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Beating the index?
              {bench?.daysCollected ? ` · ${bench.daysCollected}d sampled` : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("bench", "/api/benchmark", (v: any) => setBench(v), true)
              }
              at={at.bench ?? null}
              source={bench?.day ?? null}
              busy={!!busy.bench}
            />
          </>
        }
      ...
      <Collapse
        ...
        head={
          <>
            <span>Trends{trends?.days ? ` · ${trends.days}d` : ""}</span>
            <Refresh
              onClick={() =>
                pull(
                  `trends:${grain}`,
                  `/api/trends?grain=${grain}`,
                  (v: any) => setTrends(v),
                  true,
                )
              }
              at={at[`trends:${grain}`] ?? null}
              busy={!!busy[`trends:${grain}`]}
            />
          </>
        }
      >
        <TrendGrid
          ...
          onGrain={(g) => {
            setGrain(g);
            pull(`trends:${g}`, `/api/trends?grain=${g}`, (v: any) =>
              setTrends(v),
            );
          }}
        />
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Google AI org
              {people?.roster?.length ? ` · ${people.roster.length}` : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("people", "/api/people", (v: any) => setPeople(v), true)
              }
              at={at.people ?? null}
              source={people?.cachedAt ?? null}
              busy={!!busy.people}
            />
          </>
        }
      ...
      <Collapse
        ...
        head={
          <>
            <span>Did it move anything?</span>
            <Refresh
              onClick={() =>
                pull("corr", "/api/correlate", (v: any) => setCorr(v), true)
              }
              at={at.corr ?? null}
              busy={!!busy.corr}
            />
          ...
          <>
            <span>
              ...
              {catalog?.plugins?.length
                ? ` · ${catalog.plugins.length} packages`
                : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("catalog", "/api/plugins", (v: any) => setCatalog(v), true)
              }
              at={at.catalog ?? null}
              busy={!!busy.catalog}
            />
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Plugin findings{" "}
              {sources?.plugins?.findings?.length
                ? `· ${sources.plugins.findings.length}`
                : ""}
            </span>
            <Refresh
      ...
      >
        {sources ? (
          <Plugins findings={sources.plugins?.findings ?? []} />
        ) : (
          <Skel rows={5} />
        )}
      </Collapse>
      ...
      <Collapse
        ...
        head={
          <>
            Site health{" "}
            {sources?.errors?.rows?.length
              ? `· ${sources.errors.rows.length} issues`
              : ""}
          </>
        }
      >
          ...
          <>
            <span>
              ...
              {people?.people?.length
                ? ` · ${people.people.filter((p: any) => p.latest?.reactable).length} reactable`
                : ""}
            </span>
            <Refresh
              onClick={() =>
        ...
        head={
          <>
            <span>
              PR board
              {board?.prs?.length
                ? ` · ${board.prs.filter((p: any) => p.actionRequired).length} need you`
                : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("board", "/api/board", (v: any) => setBoard(v), true)
              }
              at={at.board ?? null}
              source={board?.cachedAt ?? null}
              busy={!!busy.board}
            />
          </>
        }
      ...
      <Collapse
        ...
        head={
          <>
            ...
            <Refresh
              onClick={() =>
                pull("eco", "/api/ecosystem", (v: any) => setEco(v), true)
              }
              at={at.eco ?? null}
              busy={!!busy.eco}
            />
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Article web{web?.nodes?.length ? ` · ${web.nodes.length}` : ""}
            </span>
            <Refresh
              onClick={() =>
                pull("web", "/api/articles", (v: any) => setWeb(v), true)
              }
              at={at.web ?? null}
              busy={!!busy.web}
            />
      ...
      <Collapse
        ...
        head={
          <>
            <span>
              Audience clock
              {audience?.zones?.length
                ? ` · ${audience.zones.length} zones`
                : ""}
            </span>
            <Refresh
              onClick={() =>
