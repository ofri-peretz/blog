...
export default function League() {
  const { data, at, busy, refresh } = useCachedSection<any>("league", "/api/league", () => ({ climb: null, tables: [] }));
  const c = data?.climb;
  return (
    <main className="mx-auto flex max-w-5xl flex-col gap-8 px-5 py-10 pb-24">
      <header className="flex flex-col gap-1">
        <Link href="/" className="font-mono text-[11px] uppercase tracking-[0.14em] text-[var(--muted-foreground)] hover:text-[var(--primary)]">← control room</Link>
        <div className="flex flex-wrap items-baseline gap-4">
          <h1 className="text-[28px] font-semibold tracking-tight">The climb</h1>
          <button onClick={() => refresh()} disabled={busy} className="font-mono text-[11px] uppercase tracking-wider text-[var(--muted-foreground)] hover:text-[var(--primary)]">{busy ? "crawling…" : "refresh"}</button>
          {at ? <span className="font-mono text-[10px] text-[var(--muted-foreground)]">read {new Date(at).toLocaleString()}</span> : null}
        </div>
        <p className="max-w-3xl text-[14px] text-[var(--muted-foreground)]">
          Every author with an article in dev.to&rsquo;s top 500 of the last 30 days, plus the top 300 in #security, #javascript, #node and #ai, ranked by reactions. dev.to publishes no author score; this is the one it does publish, article by article, added up.
        </p>
      </header>
      ...
      {!c ? (
        ...
        <>
          <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {tiles(c).map(([k, v, s]) => (
              <div key={k} className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3">
                ...
                <div className="mt-1 text-[11px] text-[var(--muted-foreground)]">{s}</div>
              </div>
            ))}
          </section>
          ...
          {c.nextUp.length ? (
            <section>
              <h2 className="font-mono text-[11px] uppercase tracking-[0.12em] text-[var(--muted-foreground)]">Next up — the five directly above us</h2>
              <Table rows={c.nextUp} gap />
            </section>
          ) : null}

          <section>
            <h2 className="font-mono text-[11px] uppercase tracking-[0.12em] text-[var(--muted-foreground)]">The top 25</h2>
            <Table rows={c.top} />
          </section>
          ...
          <p className="text-[12px] text-[var(--muted-foreground)]">Sample-bound: an article outside these pages is invisible here. A single viral post moves an author a hundred places in a week, which is why articles and reactions per article print beside rank.</p>
        </>
      )}
    </main>
  );
  ...
  return (
    <div className="mt-2 overflow-x-auto rounded-lg border border-[var(--border)]">
      <table className="w-full text-[12px]">
        <thead><tr className="font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--muted-foreground)]"><th className="px-2 py-1.5 text-right">#</th><th className="px-2 py-1.5 text-left">author</th><th className="px-2 py-1.5 text-right">reactions</th><th className="px-2 py-1.5 text-right">comments</th><th className="px-2 py-1.5 text-right">articles</th><th className="px-2 py-1.5 text-right">{gap ? "gap to us" : "rx / article"}</th><th className="px-2 py-1.5 text-left">{gap ? "" : "tags"}</th></tr></thead>
        <tbody>{rows.map((l) => (
