...
export async function GET() {
    ...
    .map((c: any) => {
      ...
      return {
        ...
        blockers: [
          ...
          // Only a MEASURED low count is a blocker. Three quarters of the
          // board has no outsideMerges reading at all, and `?? 0` turned every
          // one of those into "slow door" — an invented number, not a finding.
          ...(c.outsideMerges != null && c.outsideMerges < 5
  ...
  return NextResponse.json({
    ...
    customers,
    /**
     * The ranked candidates travel INSIDE `sectors` only. The first draft also
     * sent them flat, and with 4,079 rows that was the same 3 MB twice — a
     * 6 MB answer for a page that reads the nested copy and the count. Anyone
     * needing the flat ranking can concatenate `sectors[].items`; the sort
     * inside each sector is the global one.
     */
