...
// One `gh api` round-trip per PR times five endpoints; a cold sweep of a dozen
// PRs takes well over the default function budget.
...
export async function GET(req: Request) {
  // "Not cached in customers.json" is about the FILE — a stored copy nobody
  // re-asks is wrong forever. A 15-minute disk cache is a different thing: it
  // makes navigating back to the page free, the same budget /api/board uses
  // for the same GitHub sweep, and the section's own refresh button sends
  // `?refresh=1` to re-ask GitHub now. Without it every visit paid the whole
  ...
  return NextResponse.json({
    ...
    cachedAt: hit.at,
    cached: !hit.fresh,
    // A stale answer served because GitHub failed just now is still an answer,
    // but the page must be able to say so rather than present it as live.
