...
async function build() {
  ...
  const [followers, commenters, refs] = await Promise.all([
    sb("devto_followers?select=username,onboarding&limit=5000").catch(() => []),
    sb("v_devto_comments?select=author,direction,created_at,our_reply_at&direction=eq.in&limit=5000").catch(() => []),
