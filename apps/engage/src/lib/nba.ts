...
export function rankActions(
  ...
  now = Date.now(),
  coreN = CORE_N,
  /** author → league rank for the authors around us on the ladder; their readers react in our tags. */
  ladder: Map<string, number> = new Map(),
): NbaRow[] {
  ...
  const core = new Set(
    (graph?.nodes ?? [])
      ...
      .sort(
        (a, b) =>
          b.degree - a.degree || b.in - a.in || a.id.localeCompare(b.id),
      )
      .slice(0, coreN)
      .map((n) => n.id),
  );
  ...
  for (const a of acted)
    lastActed.set(a.author, Math.max(lastActed.get(a.author) ?? 0, a.at));
  ...
  for (const t of threads) {
    ...
    let score = 0;
    const why: string[] = [];
    if (to.has(t.author) && !from.has(t.author)) {
      score += 3;
      why.push("closes a mutual tie — they commented on us, no tie back yet");
    } else if (!from.has(t.author) && !to.has(t.author)) {
      score += 1;
      why.push("new author");
    } else why.push("keeps a conversation going");
    if (core.has(t.author)) {
      score += 2;
      why.push(`core node (top ${coreN})`);
    }
    ...
    if (t.replyToUs) {
      score += 2;
      why.push("replied directly to us");
    }
    if (ladder.has(t.author)) {
      score += 1.5;
      why.push(
        `ladder neighbour, rank ${ladder.get(t.author)} — the climb's own readers`,
      );
    }
    const weeks = Math.min(4, Math.floor((t.ageDays ?? 0) / 7));
    if (weeks) {
      score += 0.5 * weeks;
      why.push(`waited ${t.ageDays}d`);
    }
    rows.push({
      source: "thread",
      index: t.index,
  ...
  for (const it of items) {
    ...
    let score = 0;
    const why: string[] = [];
    if (cooling(it.author)) {
      rows.push({
        source: "item",
        index: it.index,
        author: it.author,
        score: 0,
        why: `cooldown — acted on @${it.author} in the last ${COOLDOWN_DAYS}d`,
      });
      continue;
    }
    if (to.has(it.author) && !from.has(it.author)) {
      score += 3;
      why.push("closes a mutual tie — they commented on us first");
    } else if (!from.has(it.author) && !to.has(it.author)) {
      score += 1;
      why.push("new author");
    } else why.push("existing tie");
    if (core.has(it.author) && !from.has(it.author)) {
      score += 2;
      why.push(`untouched core node (top ${coreN})`);
    }
    if (ladder.has(it.author)) {
      score += 1.5;
      why.push(
        `ladder neighbour, rank ${ladder.get(it.author)} — the climb's own readers`,
      );
    }
