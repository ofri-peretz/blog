...
`{ "$ne": null }` is not a typo or a missing `await`. It's user-controlled request data flowing directly into a MongoDB query with no sanitization — operator injection, a class generic linters can't see and code review consistently misses, because the type is `any`, the test posts a string, and the build stays green. In a benchmark of 700 AI-generated Node.js functions, **96% of database-layer outputs from the flagship Gemini model contained a security vulnerability** — this exact class of bug is a large share of why.

Here are four specific bugs your team has almost certainly shipped — three the linter blocks outright, and one gap in the ruleset that review discipline still has to cover.
...
`$where` runs arbitrary server-side JavaScript inside the MongoDB process — reaching it with any user-controlled string, hand-written or smuggled through `populate()`, is remote code execution (CWE-943, CVSS 9.8), not a filter bug.
...
**ESLint rule:** [`no-unsafe-where`](https://eslint.interlace.tools/docs/security/plugin-mongodb-security/rules/no-unsafe-where) + [`no-unsafe-query`](https://eslint.interlace.tools/docs/security/plugin-mongodb-security/rules/no-unsafe-query), catching a CWE-943 injection (CVSS 9.8 for RCE via `$where`)
...
> Related reads: [The 30-Minute Security Audit: I Ran It on 140 Gemini-Written Functions. 102 Shipped Vulnerable.](https://ofriperetz.dev/articles/the-30-minute-security-audit-onboarding-a-new-codebase) · [The #1 ESLint Security Plugin Has 1.5M Downloads and Caught 0 of My 40 Vulnerabilities](https://ofriperetz.dev/articles/benchmark-17-eslint-security-plugins-compared)
...
```javascript
// ✅ Explicit projection — only return what the caller needs
const user = await User.find({ role: "member" }, { name: 1, email: 1, _id: 0 });
```

The `_id: 0` isn't decoration — MongoDB includes `_id` in the result by default even with an otherwise-selective projection, so it's the one field you have to exclude explicitly if the caller shouldn't see it.
...
## How to catch MongoDB injection in CI with eslint-plugin-mongodb-security

`eslint-plugin-mongodb-security` is the only ESLint plugin built specifically for MongoDB/Mongoose codebases — 16 rules, each mapped to a CWE and the relevant CVE. One config block catches Bugs 1 through 3 above:
...
**What you get on a typical vulnerable file:** ran the `recommended` preset over this 19-line Express login route — reproduce it yourself:
...
Reading the seven findings by block: the `users.findOne` (lines 6-8) accounts for one warning (`no-select-sensitive-fields` — no projection, and this is the query actually capable of returning `passwordHash`) and two errors (`no-unsafe-query` on `username`, then on `password`). The `orders.find()` (lines 13-14) accounts for the remaining two warnings (`no-select-sensitive-fields` again, plus `no-unbounded-find` — two independent rules firing on one unguarded call, not double-counting) and one error (`no-unsafe-where` on the `$where` template literal). The line 2 connection string is the fourth and final error, on its own. Four errors, three warnings, seven total — the errors are exactly what a CI gate should block on merge; the warnings are the data-exposure class the `recommended` preset flags but doesn't block on.
...
For the onboarding workflow that pairs this with a broader static analysis protocol, see: [The 30-Minute Security Audit: I Ran It on 140 Gemini-Written Functions. 102 Shipped Vulnerable.](https://ofriperetz.dev/articles/the-30-minute-security-audit-onboarding-a-new-codebase).

---

**`{ "$ne": null }` is the password that bypasses every MongoDB auth handler above — the linter is the one reviewer that rejects it on every save, paste, and AI completion.**

**What's the MongoDB query security bug that actually bit your team — a `$where` left in, a `req.body` that turned into an operator, a connection string committed to `.env.example`?** Drop it in the comments. The next person debugging MongoDB operator injection at 2 AM will thank you.
