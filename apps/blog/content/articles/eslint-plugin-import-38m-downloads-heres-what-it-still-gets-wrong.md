...
| Dynamic `require()` noise hiding the dangerous case | Keep both `import/no-dynamic-require` and [`security/detect-non-literal-require`](https://dev.to/ofri-peretz/eslint-plugin-security-is-unmaintained-heres-what-nobody-tells-you-96h) on, require a reason on every disable |
