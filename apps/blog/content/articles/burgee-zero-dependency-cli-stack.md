---
title: "burgee Installs 9 Packages Where a commander Stack Installs 70. Like for Like, It Weighs 2.4× More."
...
That is the flattering row. [Part 1](https://ofriperetz.dev/articles/eslint-plugin-dependency-weight) counted ESLint plugin trees and printed the rows against my plugins; same method, same rule here.
...
Keep only layers where burgee's drop-in scores what the incumbent scores on its own test suite (burgee's compat oracle runs them): commander, chalk, ora, `@inquirer/core`, string-width, signal-exit. **28 packages, 662 KB.** burgee still installs all nine, 1,577 KB. **Like for like, burgee is 2.4× heavier on disk**, while the count stays 28 against 9 and the maintainers 14 against one.
...
One account is also a concentration. Mid-measurement, the registry received burgee@0.10.0 at 05:34 UTC, then the closeout and linegauge it requires at 05:39 and 05:42; for eight minutes `npm i burgee` failed with `ETARGET`. And the family declares Node 24, where the incumbent stack's floor is Node 22.18. If you support Node 22, the incumbents are your option.
...
node -e "const l=require('./package-lock.json').packages,m=new Set(),u=[];Promise.all(Object.keys(l).filter(k=>k.startsWith('node_modules/')).map(k=>fetch('https://registry.npmjs.org/'+k.split('node_modules/').pop().replace('/','%2F')).then(r=>r.json()).then(d=>(d.maintainers||[]).forEach(x=>m.add(x.name))).catch(()=>u.push(k)))).then(()=>console.log(m.size,'unfetched',u.length))"
```
