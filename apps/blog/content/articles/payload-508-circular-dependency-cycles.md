...
Class instantiation in method bodies is fine (lazily resolved); it is only top-level evaluation — module-scope `const`, `let`, or inline class fields that execute at class _definition_ time — that breaks.
