...
tier: "T3"
cover_image: "https://ofriperetz.dev/cdn/blog-cover-image/no-cycle-cache-poisoning-at-scale.jpg?v=b2"
social_image: "https://ofriperetz.dev/cdn/blog-cover-image/no-cycle-cache-poisoning-at-scale-og.jpg?v=b2"
...
We benchmark `import-next/no-cycle` against `import/no-cycle` (from `eslint-plugin-import`) and oxlint's native Rust port on next.js (131K stars, 14,556 source files in the repo; 2,363 of them lintable under the bench's `packages/**` glob). The two ESLint plugins agreed: **0 cycles found**. oxlint disagreed: **17 cycles found**.
...
**Skip to:** [Why the cache poisons itself](#why-the-cache-poisons-itself) · [The narrow-vs-wide smoking gun](#the-narrow-vs-wide-scope-smoking-gun) · [The fix](#the-fix) · [The 30-second test](#the-30-second-test--run-this-on-your-own-repo) · [Why AI code makes it worse](#why-ai-generated-code-makes-this-worse)
...
This piece is about _detecting_ cycles, not what they are — if you need the primer on why [a circular dependency passes every toolchain check](https://ofriperetz.dev/articles/circular-dependencies-in-javascript-explained) in the first place, start there. Every cycle-detection algorithm has the same shape:
...
This is the whole diagnostic, copy-paste ready. Run it against whatever cycle detector your CI trusts **today** — lint the whole repo, then lint your gnarliest subdirectory, and compare the two counts:

```bash
# 1. Your CURRENT detector, whole repo — note the no-cycle count
npx eslint 'src/**/*.{ts,tsx}'

# 2. Same detector, one dense subdirectory
#    (your biggest barrel tree / the dir your LLM keeps editing)
npx eslint 'src/<your-gnarliest-subdir>/**/*.{ts,tsx}'

# If step 2 reports MORE cycles than step 1, the wide scope is hiding
# cycles — you have this class of bug.
# (A correct detector never reports FEWER cycles for a superset scope than
#  for its subset. A subset finding more is the signature — and it only
#  shows up on a detector that still truncates-then-caches.)

# 3. The repair, not the probe: install the patched detector and re-run wide.
#    On >=2.3.6 the whole-repo count jumps up to INCLUDE the cycles the subset
#    was surfacing — a truncated search no longer poisons the cache, so the
#    wide scope stops hiding them.
npm i -D eslint-plugin-import-next
npx eslint 'src/**/*.{ts,tsx}'
```

That single comparison — subset finds more than the whole — is the signature, and it only appears on a detector that still has the bug. It works on _any_ cycle detector, not just ours; installing the patched `import-next` is what makes it disappear. It's also where AI-scaffolded code lights up hardest, for reasons I get into [below](#why-ai-generated-code-makes-this-worse).

## What `eslint-plugin-import` does instead

When you've found a real bug, it's worth checking how peers in the same landscape modeled the problem. The long-standing `import/no-cycle` rule (from `eslint-plugin-import`) uses a fundamentally different approach:
...
The uncomfortable part: the detector doesn't error. It returns **0**, the build goes green, and the consensus of two linters agrees with it. If you let an assistant scaffold modules and trust a green `no-cycle` run, you are trusting exactly the number this bug fabricates. Run [the 30-second test](#the-30-second-test--run-this-on-your-own-repo) against its gnarliest barrel directory — that's where the cascade has the most room to grow, and where AI-scaffolded code makes the gap widest — then upgrade to the fixed detector and watch that green `0` turn into the real count.
...
# 2. Reproduce the buggy CLASS. Setting a finite maxDepth on the PATCHED
#    plugin will NOT bring the cascade back — its cache-honesty guard is
#    exactly what kills it (a truncated search no longer poisons the cache,
#    at any cap). So pin a PRE-FIX build (default maxDepth was 10) and lint
#    wide, then subset — same eslint.config.mjs registration as above:
npm i -D 'eslint-plugin-import-next@<2.3.6'
npx eslint 'src/**/*.{ts,tsx}'   # buggy: wide
npx eslint 'src/orders/**'       # buggy: subset

# 3. Upgrade to the patched default and re-run both — the gap closes:
npm i -D eslint-plugin-import-next@latest
npx eslint 'src/**/*.{ts,tsx}'   # patched: wide
npx eslint 'src/orders/**'       # patched: subset

# Record four numbers: (buggy-wide, buggy-subset, patched-wide, patched-subset).
# The depth-cascade signature is buggy-wide < buggy-subset, with
# patched-wide == patched-subset (both now report the seeded cycle the buggy
# wide run hid). Scaffold deep enough: a handful of files won't cascade — the
# wide scope needs enough sourceFile-blind searches to poison a node before
# its own short cycle is walked (the same reason a 33-file subtree stays clean).
```
