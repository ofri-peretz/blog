...
published: true
date: 2026-02-08
published_at: "2026-02-08T03:37:21.039Z"
cover_image: "https://ofriperetz.dev/cdn/blog-cover-image/benchmark-microsoft-sdl-vs-interlace.jpg?v=b2"
social_image: "https://ofriperetz.dev/cdn/blog-cover-image/benchmark-microsoft-sdl-vs-interlace-og.jpg?v=b2"
tags:
  - "security"
  - "eslint"
  - "devsecops"
...
Your team added `@microsoft/eslint-plugin-sdl` to CI, the pipeline went green, and someone checked "security linter" off the list. Meanwhile **37 of the 46 findings** the domain plugins raise on the same Node backend file — SQL injection, path traversal, prototype pollution, ReDoS — sit on lines SDL has no rule for. The build is still green. The SQL injection is still there.
...
**Skip to:** [the numbers](#what-the-numbers-actually-are) · [the 46 findings by rule ID](#what-a-node-backend-needs-instead) · [three bugs, end to end](#three-bugs-end-to-end-what-sdl-cant-see) · [the AI angle](#the-ai-angle-assistants-reintroduce-exactly-what-sdl-cant-see) · [reproduce it](#methodology--reproduce-it)

## What the numbers actually are

On one Node file with 12 vulnerability classes, `@microsoft/eslint-plugin-sdl@1.1.0` (measured 2026-05-31; every version pinned in the methodology) running its `recommended` config caught **5** (and only **3** of those came from its own `@microsoft/sdl/*` rules). The domain plugins caught **46** — same file. And you'd already told the team the backend was covered by Microsoft's Security Development Lifecycle, because the build went green. Your SQL injection, your path traversal, your unsafe deserialization — all of it walked straight past the linter you trusted, and the pipeline never went red to warn you.

That gap isn't a quality verdict, and it isn't Microsoft shipping a bad tool. `@microsoft/eslint-plugin-sdl` is 17 rules (at v1.1.0) distilled from the SDL standard, and it's a _good_ tool — for the surface it was built for. Look at the rule list and the result is obvious: SDL was built to harden **frontends** (Angular, Electron, the DOM), not Node backends. Point it at an API and most of its rules have nothing to match.

One thing before the table: I wrote the four domain plugins _and_ the fixture, so you should be suspicious of me — the two configs at the bottom exist so you don't have to take my word for any of this.
...
**What SDL can't automate, and why.** SDL's standard also covers things no linter can touch — threat modeling, architectural security review, penetration testing, access control design — work that needs runtime context and human judgment a static tool structurally lacks. That isn't a weakness of SDL; it's a reminder that "CI is green" and "the backend is secure" are different claims.
...
SDL _namespaced_ rules (`@microsoft/sdl/*`) caught **3**: `no-inner-html`,
...
namespace; they run `sdl.configs.recommended`, and that config _also_ turns on
...
_registers_ `eslint-plugin-security` as a plugin but never switches its rules to
...
findings** — the SQLi, path traversal, object injection, ReDoS, weak hashing,
timing, dynamic-require, and deprecated-Buffer rows — have **no SDL rule at all**.
...
collapse. `userId` _reads_ like an integer from a typed route param, the diff is
...
a warning, _absent from the output entirely_.
...
```js
// resolve first, then check the prefix WITH the trailing separator
...
The `+ path.sep` is load-bearing: a prefix check against bare `uploadsDir` waves
through a sibling directory like `uploads-evil/payload`. Half the path-traversal
"fixes" you'll meet in code review carry exactly that bug — resolve the full path
first, then compare against the directory _plus_ the separator.
...
harden a surface this repo doesn't have. The rules that _would_ have caught the
...
findings SDL's recommended config has no rule for are the _modal_ mistakes an LLM
...
in nearly half its functions. These are the _exact_ CWE classes SDL's recommended
...
_registers_ `eslint-plugin-security` but does **not** enable its rules at `error`
...
at a 49–73% clip, which is why the _layer_ — not the author — is the thing to fix.
